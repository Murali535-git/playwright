...
test('Validating the Web API methods', async ({ page }) => {

    await page.goto('https://rahulshettyacademy.com/AutomationPractice/');  
});

test('Online Web E2E Scenario with API login', async ({page})=>{

    page.addInitScript(value => {
        window.localStorage.setItem('token', value); // Store the token in local storage
    }, token); // Pass the token as an argument to the init script
 
    await page.goto('https://rahulshettyacademy.com/client/');

    const email="murali535@gmail.com";
